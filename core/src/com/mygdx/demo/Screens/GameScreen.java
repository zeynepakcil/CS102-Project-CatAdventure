...
public class GameScreen implements Screen {
    ...
    public void handleInput(float d) {
        ...
        if (Cat.current != Cat.State.DEAD) {
            if (jumpCount <2 &&  (Gdx.input.isKeyJustPressed(Input.Keys.UP))) {
                jumpCount++;
            ...
            if (Gdx.input.isKeyPressed(Input.Keys.LEFT) && cat.box2d.getLinearVelocity().x >= -2)
                cat.box2d.applyLinearImpulse(new Vector2(-0.1f, 0), cat.box2d.getWorldCenter(), true);
            if (jumpCount <2 &&  (Gdx.input.isKeyJustPressed(Input.Keys.W))) {
                jumpCount++;
                cat.box2d.applyLinearImpulse(new Vector2(0, 4f), cat.box2d.getWorldCenter(), true);
            }
            if (Gdx.input.isKeyPressed(Input.Keys.D) && cat.box2d.getLinearVelocity().x <= 2)
                cat.box2d.applyLinearImpulse(new Vector2(0.1f, 0), cat.box2d.getWorldCenter(), true);
            if (Gdx.input.isKeyPressed(Input.Keys.S) && cat.box2d.getLinearVelocity().x >= -2)
                cat.box2d.applyLinearImpulse(new Vector2(-0.1f, 0), cat.box2d.getWorldCenter(), true);
            if(cat.box2d.getLinearVelocity().y==0){
                jumpCount=0;
            }
            }
    ...
    public boolean gameOver() {
        if (Cat.current == Cat.State.DEAD && cat.getStateTimer() > 3 || LevelState.worldTimer ==0)
            return true;
        return false;
    }
